...
df_greeks = pd.read_csv(path_save)   
 
df_greeks["Unnamed: 0"]=pd.to_datetime(df_greeks["Unnamed: 0"],format='%Y-%m-%d %H:%M:%S.%f%z')
df_greeks["oi_direction"] = df_greeks["Put_OI"] - df_greeks["Call_OI"] 
df_greeks.rename(columns={'Unnamed: 0': 'Timestamp'}, inplace=True)
# df_greeks['Timestamp'] = df_greeks['Unnamed: 0'].dt.strftime('%r')

# Get todays data
Today = datetime.datetime.now().date()
df_greeks = df_greeks[df_greeks['Timestamp'].dt.date == Today]
...
theta_chart = dashboard_functions.multi_line_chart("Theta"	,df_greeks,"Timestamp","c_theta"	,"p_theta")
delta_chart = dashboard_functions.multi_line_chart("Delta"	,df_greeks,"Timestamp","c_delta"	,"p_delta")
vega_chart  = dashboard_functions.multi_line_chart("Vega"	,df_greeks,"Timestamp","c_vega"	,"p_vega")
